/* ═══════════════════════════════════════════════════
   a-pets-new.js  –  Owner & Pets page logic
   DB schema:
     user → UserID, UserName, OwnerFName, OwnerLName,
             ContactNum, Address, Email, Password, Role
     pet  → PetID, UserID, Name, Species, Color, Breed,
             Birthdate, LastVisit, Gender, photo
═══════════════════════════════════════════════════ */
'use strict';
...
function renderTable(list) {
    ...
    // Show no-owner image only when there are truly NO owners at all, not on search miss
    if (owners.length === 0) {
...
function selectOwner(userId) {
    ...
    /* Switch panel — activate two-column layout */
...
/* ══════════════════════════════════════
   ADD / EDIT PET MODAL
══════════════════════════════════════ */
let petFormOriginal = null;  // snapshot for unsaved-change detection
let petFormMode = null;      // 'add' | 'edit'
...
function clearPetForm() {
    ['fPetId','fPetName','fPetBirthdate','fPetAge','fPetBreed','fPetColor','fPetLastVisit','fPetOwnerDisplay'].forEach(id => {
...
document.getElementById('fPetBirthdate').addEventListener('change', function () {
    ...
    /* Update emoji preview based on species */
...
/* ══════════════════════════════════════
   PET PHOTO ZOOM + DRAG (circle preview)
══════════════════════════════════════ */
...
function submitPetForm() {
    ...
    const species   = document.getElementById('fPetSpecies').value;
    ...
    if (!name || !birthdate || !species || !breed || !gender) {
...
function submitOwnerForm() {
    ...
    if (uid) {
        ...
        /* Refresh owner strip if this owner is currently selected */
...
document.querySelectorAll('.modal-overlay').forEach(overlay => {
    overlay.addEventListener('click', e => {
        ...
        /* intercept pet/owner form closes */
...
/* ══════════════════════════════════════
   IMAGE LIGHTBOX  (free pan + zoom)
   transform-origin: 0 0
   transform: translate(tx,ty) scale(s)
   zoom anchored at cursor/pinch midpoint
══════════════════════════════════════ */
(function () {
